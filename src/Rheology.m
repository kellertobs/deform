...
% Rheology    DEFORM: Update material rheology
%
% [CTX] = Rheology(MP,CTX)
%
...
Eta0  =  PROP.Eta(Mat) .* 10.^(-MP.pert.*CTX.INIT.PertEta);
...
if strcmp(RHEO.Plasticity,'ON')
    ...
    Coh    =  Coh   .* max(0.1,RHEO.DmgDCoh  .^DMG);
    Frict  =  Frict .* max(0.1,RHEO.DmgDFrict.^DMG);
...
EtaP        =  10.^SmoothField(log10(EtaP),CTX.SL.Smooth,1,CTX.FE,'IP');
