% preview model output as time series
    ...
    if ischar(varargin{n})
        if (strcmp(varargin{n}(1:3),'sub') || strcmp(varargin{n}(1:3),'add') ...
         || strcmp(varargin{n}(1:3),'div') || strcmp(varargin{n}(1:3),'mul'))
            ...
            n    = n+1;
            name2 = varargin{n};
        elseif strcmp(varargin{n}(1:3),'max')
            n = n+1;
            maxval = varargin{n};
        elseif strcmp(varargin{n}(1:3),'min')
            n = n+1;
            minval = varargin{n};
        elseif strcmp(varargin{n}(1:3),'tit')
            n = n+1;
            titlestr = varargin{n};
        elseif strcmp(varargin{n}(1:3),'tag')
            n = n+1;
            tagstr = varargin{n};
...
for frame=start:step:stop
    ...
    field = min(maxval,max(minval,field));
    ...
    if ~exist('titlestr','var'); titlestr = name; end
    title([titlestr,'; ',num2str(time,'%1.2f'),tunit],'Interpreter','Latex','FontSize',18); drawnow;
    
    if printfig
        if ~exist('tagstr','var'); tagstr = name; end
        print(fh,format,res,rend,[CTX.IO.DataDir '/' CTX.IO.RunID '/' CTX.IO.RunID '_' tagstr '_' num2str(frame)],'-loose');
    end
end
